document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('saveSendingIdBtn').addEventListener('click', function() {
        ...
        const randomIdInput = document.getElementById('randomIdInput').value;

        if (sendingId) {
            // ランダムIDが入力されていなければ自動で生成
            const idToSave = randomIdInput || generateRandomString(20);

            // ランダムIDと送信先IDをchrome.storageに保存
            chrome.storage.local.set({ key: idToSave, sendingId: sendingId }, function() {
                console.log('ランダムIDと送信先IDが保存されました:', idToSave, sendingId);
                // 送信先IDが保存された後、入力欄とボタンを隠す
                document.getElementById('sendingIdSection').style.display = 'none';
                document.getElementById('savedIdSection').style.display = 'block';
                // 保存されたIDと入力された送信先IDを表示
                document.getElementById('savedIdOutput').textContent = '生成されたID: ' + idToSave;
                document.getElementById('inputtedIdOutput').textContent = '送信先ID: ' + sendingId;
            });
        } else {
